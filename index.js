...
async function accountLogin(state, enableCommands = [], prefix, admin = []) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == userid)) {
    api.changeNickname(`𝗔𝗨𝗧𝗢𝗕𝗢𝗧`, threadID, userid);
...
.then(response => {
    fs.writeFileSync(gifPath, response.data);					  return api.sendMessage(`✅ 𝗚𝗥𝗢𝗨𝗣 𝗖𝗢𝗡𝗡𝗘𝗖𝗧𝗜𝗢𝗡 𝗦𝗨𝗖𝗖𝗘𝗦! \n➭ Bot Prefix: ${prefix}\n➭ Admin: ‹${admin}›\n➭ Facebook: ‹https://www.facebook.com/mark.dev69›\n➭ Use ${prefix}help to view command details\n➭ Added bot at: ⟨ ${time} ⟩〈 ${thu} 〉`, event.threadID,
                    );
                })
                .catch(error => {
                    console.error('Error fetching GIF:', error);
									 ...
									 if (event.body !== null) {
										...
										if (event.body !== null) {
											...
											//*Auto Download Google Drive here By Jonell Magallanes//*        if (event.body !== null) {
												(async () => {
													const fs = require('fs');
													...
													while ((match = gdriveLinkPattern.exec(event.body)) !== null) {
														...
														try {
															...
															await api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖽𝗈𝗐𝗇 𝖦𝗈𝗈𝗀𝗅𝖾 𝖣𝗋𝗂𝗏𝖾 𝖫𝗂𝗇𝗄 \n\n𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴: ${fileName}\n\n𝗔𝗨𝗧𝗢𝗕𝗢𝗧 𝙱𝚢 𝙼𝚊𝚛𝚔𝚢`, attachment: fs.createReadStream(destPath) }, event.threadID);
          ...
          if (youtubeLinkPattern.test(videoUrl)) {
            youtube.getVideo(videoUrl)
              .then(video => {
                ...
                file.on('finish', () => {
                  file.close(() => {
                    api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖸𝗈𝗎𝖳𝗎𝖻𝖾 \n\n𝗔𝗨𝗧𝗢𝗕𝗢𝗧 𝙱𝚢 𝙼𝚊𝚛𝚔𝚢`, attachment: fs.createReadStream(filePath) }, event.threadID, () => fs.unlinkSync(filePath));
                  });
                });
              })
              .catch(error => {
									...
									if (event.body !== null) {
										...
										const downloadAndSendFBContent = async (url) => {
											try {
												...
												return api.sendMessage({ body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 𝖵𝗂𝖽𝖾𝗈\n\n𝗔𝗨𝗧𝗢𝗕𝗢𝗧 𝙱𝚢 𝙼𝚊𝚛𝚔𝚢", attachment: fs.createReadStream(fbvid) }, event.threadID, () => fs.unlinkSync(fbvid));
											}
											catch (e) {
